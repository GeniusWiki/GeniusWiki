 ...
 * @author Dapeng.Ni
 */
public class InstancePortalVisitor implements PortalVisitor {

	private Portal portal;

	public void fetchPortlet(int column) {
		PortalControllerAsync homeController = ControllerFactory.getPortalController();
		homeController.getDefaultPortal(column, portal.getLoadAsyncCallback());
	}
	public void saveLayout(ArrayList<String> list) {
		PortalControllerAsync homeController = ControllerFactory.getPortalController();
		homeController.updateDefaultPortal(list,portal.getSaveAsyncCallback());		
	}
