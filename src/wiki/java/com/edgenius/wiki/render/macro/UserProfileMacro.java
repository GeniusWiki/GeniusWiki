...
/**
 * User profile page - normally system internal usage
 * {userprofile:username=boo}
 * @author Dapeng.Ni
 */
public class UserProfileMacro  extends BaseMacro {
	private final static String HANDLER = NameMacroHandler.class.getName();
	
	public String[] getName() {
		return  new String[]{"userprofile"};
	}

	public void execute(StringBuffer writer, MacroParameter params) throws MalformedMacroException {
		if(params == null){
			log.error("Unexpected parameters for user profile handler. It need use name param. But given is null");
			return;
		}
		...
		obj.values.put(NameConstants.MACRO, SharedConstants.MACRO_USER_PROFILE);
	...
	public String getHTMLIdentifier() {
		return "<div aid='"+SharedConstants.MACRO_USER_PROFILE+"'>";
	}
	@Override
	protected void replaceHTML(HTMLNode node, ListIterator<HTMLNode> iter, RenderContext context) {
		resetMacroMarkup(TIDY_STYLE_BLOCK, node, iter, getMacroMarkupString(node, "userprofile"), null);
