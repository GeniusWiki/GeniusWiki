...
public class QuoteMacro extends BaseMacro{
	...
	public void execute(StringBuffer buffer, MacroParameter params) throws MalformedMacroException {
		buffer.append("<blockquote>").append(params.getContent()).append("</blockquote>");
		
	}
	@Override
	protected void replaceHTML(HTMLNode node,ListIterator<HTMLNode> iter,  RenderContext context) {
		if(node.getPair() == null){
			log.warn("Unexpect case: No close blockquote tag for " + this.getClass().getName());
			return;
		}
		
		resetMacroMarkup(TIDY_STYLE_BLOCK, node, iter, "{quote}", "{quote}");
	}
