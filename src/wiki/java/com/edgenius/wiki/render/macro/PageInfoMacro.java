...
import com.edgenius.wiki.render.handler.PageInfoHandler;
import com.edgenius.wiki.render.object.ObjectPosition;

/**
 * Display current page information, such as author, modifier etc.
 * 
 * {pageinfo:type=creator|value=name/portrait/date/status}
 * {pageinfo:type=modifier|value=name/portrait/date/status}
 * 
 * If value is not provide, it will includes name, portrait and date except status
 * @author Dapeng.Ni
 */
public class PageInfoMacro extends BaseMacro {
	
	private static final String HANDLER = PageInfoHandler.class.getName();

	//JDK1.6 @Override
	public String[] getName() {
		return new String[]{"pageinfo"};
	}
	@Override
	public String getHTMLIdentifier() {
		return "<div class='macroPageinfo'>";
	}
	//JDK1.6 @Override
	public void execute(StringBuffer buffer, MacroParameter params) throws MalformedMacroException {
		...
		obj.values.put(NameConstants.MACRO, "pageinfo");
	...
	protected void replaceHTML(HTMLNode node, ListIterator<HTMLNode> iter, RenderContext context) {
		...
		resetMacroMarkup(Macro.TIDY_STYLE_BLOCK, node, iter,  getMacroMarkupString(node, "pageinfo"), null);
