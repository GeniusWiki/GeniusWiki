...
In setup step, first step is using default option, simply click 'OK'. In 2nd step, replace `127.0.0.1:61616` with `activemq.docker.geniuswiki:61616` for activemq address. In 3rd step, choose `MySQL` database and replace '127.0.0.1:3306' with `mysql.docker.geniuswiki`. 

All data, include database, log, upload data is saved into ./docker/data directly.

Note, a potential issue to stop geniuswiki success rebooting after the setup, just stop docker `docker-compose stop` then start again.
