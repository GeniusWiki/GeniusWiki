	...
	tinymce.create('tinymce.plugins.TablePlugin', {
		init : function(ed, url) {
			...
			ed.onInit.add(function() {
				...
				if (ed && ed.plugins.contextmenu && !sc && !ec) {
					ed.plugins.contextmenu.onContextMenu.add(function(th, m, e) {
						...
						if (ed.dom.getParent(e, 'td') || ed.dom.getParent(e, 'th')) {
							...
							//Edgenius: NDPNDP - disable table embedded insert 
							//m.add({title : 'table.desc', icon : 'table', cmd : 'mceInsertTable', ui : true, value : {action : 'insert'}});
							
							m.add({title : 'table.props_desc', icon : 'table_props', cmd : 'mceInsertTable', ui : true, value : {action : 'update'}});
							m.add({title : 'table.del', icon : 'delete_table', cmd : 'mceTableDelete', ui : true});
							m.addSeparator();

							// Cell menu
							sm = m.addMenu({title : 'table.cell'});
							//Edgenius: NDPNDP - disable context menu cell properties
			...
			ed.onNodeChange.add(function(ed, cm, n) {
				...
				//EDGENIUS: disable insert embedded table 
				cm.setDisabled('table',  !!p);
		...
		_doExecCommand : function(command, user_interface, value) {
			...
			switch (command) {
				...
				case "mceTableRowProps":
					...
					if (user_interface) {
						/** Edgenius: don't open default popup, use GWT popup instead...   
				...
				case "mceInsertTable":
					if (user_interface) {
						...
						**/
						var action = value ? value.action : 0;
						if(action == 'update'){
							gwtOpenUpdateTableDialog(ed.id);
						}else{
							gwtOpenInsertTableDialog(ed.id);
						}
					}
