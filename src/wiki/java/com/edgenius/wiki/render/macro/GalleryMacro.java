...
import com.edgenius.wiki.render.handler.GalleryMacroHandler;
import com.edgenius.wiki.render.object.ObjectPosition;

/**
 * Get all image from page attachments and display on a lightbox style gallery.
 * Filter attribute support filename wildcard, and can use ";" separate mutliple filters.
 * 
 * {gallery}
 * {gallery:column=4|thumbsize=180*180|filter=*.jpg}
 * 
 * @author Dapeng.Ni
 */
public class GalleryMacro extends BaseMacro {
	
	private final static String HANDLER = GalleryMacroHandler.class.getName();
	...
	protected void replaceHTML(HTMLNode node, ListIterator<HTMLNode> iter, RenderContext context) {
		resetMacroMarkup(TIDY_STYLE_BLOCK, node, iter, getMacroMarkupString(node, "gallery"), null);
