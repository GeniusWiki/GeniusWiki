...
public class PageDAOHibernate extends BaseDAOHibernate<Page> implements PageDAO {
	...
	public Page getCurrentByUuid(String uuid) {
		List<Page> list = find(GET_CURRENT_BY_UUID,uuid);
	...
	public Page getCurrentByUnixName(String spaceUname, String unixName) {
		List<Page> list = find(GET_CURRENT_BY_UXNAME,new Object[]{spaceUname, unixName});
		if(list == null || list.isEmpty())
			return null;
		if(list.size() > 1)
	...
	public Page getByUuid(String pageUuid) {
		List<Page> pages =  find(GET_ALL_BY_UUID,pageUuid);
	...
	public Map<Integer,String> getTitlesByUuid(String uuid) {
		...
		List<Object[]> rs = find(GET_ALL_TITLE_BY_UUID,uuid);
		if(rs == null){
			return map;
		}
	...
	public Page getCurrentPageByTitle(String spaceUname, String pageUname) {
		List list = find(GET_CURRENT_BY_TITLE,new Object[]{spaceUname,pageUname});
		if(list == null || list.isEmpty())
			return null;
		if(list.size() > 1)
	...
	public List<Page> getTree(String spaceUname) {
		...
		List<Object[]> list = find(GET_TREE,spaceUname);
	...
	public List<Page> getChildren(Integer parentUid) {
		return find(GET_CHILDREN_BY_PARENTUID,parentUid);
	}
	public Integer getChildrenCount(Integer parentUid) {
		@SuppressWarnings("rawtypes")
		List list = find(GET_CHILDREN_COUNT_BY_PARENT_UID,parentUid);
	...
	public List<Page> getRemovedPagesInSpace(String spaceUname) {
		return find(GET_REMOVED_IN_SPACE,spaceUname);
	}
	...
	public List<String> getPagesUuidInSpace(String spaceUname){
		return find(GET_PAGES_UUID,new Object[]{spaceUname});
	}
	...
	public List<Page> getSpaceAllPages(String spaceUname) {
		return findByNamedParam(GET_SPACE_ALL_PAGES,"spaceUname",spaceUname);
	}

	//JDK1.6 @Override
	public long getSystemPageCount() {
		List list = find(GET_INSTANCE_PAGES_COUNT);
	...
	public long getUserAuthoredSize(String username) {
		List list = find(GET_COUNT_BY_AUTHORED,username);
		if(list != null && list.size() > 0){
			return (Long)list.get(0);
		}
		return 0;
	...
	public long getUserModifiedSize(String username) {
		List list = find(GET_COUNT_BY_MODIFIED,username);
		if(list != null && list.size() > 0){
			return (Long)list.get(0);
		}
		return 0;
	...
	public Page getPageByProgressExtLinkID(String spaceUname, String extLinkID) {
		List<Page> list = find(GET_PAGES_BY_PROGRESS_EXT_LINK_ID,new Object[]{spaceUname, extLinkID});
		if(list == null || list.size() == 0)
			return null;
		return list.get(0);
	}
	...
	public List<Page> getPinTopPagesInSpace(Integer spaceUid) {
		return find(GET_PIN_TOP_PAGES_BY_SPACEUID,spaceUid);
	}
