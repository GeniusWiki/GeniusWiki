	...
	public void invokeJob() throws QuartzException{
		...
		try{
			...
			CronTrigger trigger = TriggerBuilder.newTrigger().withIdentity(triggerKey)
					.withSchedule(CronScheduleBuilder.cronSchedule(Global.VersionCheckCron)).build();
	...
	public void cancelJob() throws QuartzException{
		try {
			if(scheduler.checkExists(triggerKey)){
				...
				log.info("Version check job trigger is cancelled." );
			}
			
			JobKey jobKey = new JobKey(JOB_NAME, Scheduler.DEFAULT_GROUP);
			if(scheduler.checkExists(jobKey)){
				scheduler.deleteJob(jobKey);
				log.info("Version check job is removed." );
			}
		} catch (SchedulerException e) {
