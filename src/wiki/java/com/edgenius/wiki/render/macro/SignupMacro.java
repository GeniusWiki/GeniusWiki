...
/**
 * Open signup dialog. image could be an attachment from page or default "on" means use system default image.
 * Normally title and image won't use together.
 * 
 * {signup:title=Please sign up|image=on}
 * @author Dapeng.Ni
 */
public class SignupMacro extends BaseMacro {
	...
	public void execute(StringBuffer buffer, MacroParameter params) throws MalformedMacroException {
		...
		obj.values.put(NameConstants.MACRO, SharedConstants.MACRO_SIGNUP);
		...
		if(ps != null){
			...
			if(NameConstants.ON.equalsIgnoreCase(img)){
				...
				img = WebUtil.getWebConext() + "static/images/signup.png";
