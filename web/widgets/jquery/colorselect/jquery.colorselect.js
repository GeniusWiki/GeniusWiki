...
(function($) {
    ...
    //NDPNDP
    if (!StrFormatNoEncode || typeof (StrFormatNoEncode) != "function") {
        var StrFormatNoEncode = function(temp, dataarry) {
  ...
  //NDPNDP
    ...
    $.fn.colorselect = function(option) {        
        ...
        var blanka = '<a href="#" id="{1}-{0}" key="{2}" hidefocus="on"><em><span style=" text-align:center; width:46px;padding-top:2px; height:11px;border:solid 1px #8B7B8B;" unselectable="on">'+i18n.colorselect.none+'</span></em></a>'
        var blank = [];
        blank.push("ffffff");
        blank.push(id);
        blank.push(-1);
        ...
        //NDPNDP:to fix creating new div every popup.
        var div = $("#div"+id);
        if(div.length == 0){
	        // var innerdiv = $("<div />").addClass("centerdiv").html(result);
	        // html.push('</div>');
	        div = $("<div />").addClass("x-color-palette").css({
	            position: "absolute",
	            "z-index": "999",
	            "overflow": "auto",
	            width: options.width,
	        ...
	        $("#div" + id + "  a").click(function(e) {
	            ...
	            //get refresh element rather than from outside as may be re-generated (dialog close, then open, a new leftdiv is created)
	            var leftdiv = $("#"+leftId);
